"""
app.py — ET AI-Native News Platform
Main Streamlit application entry point.

Three modules:
  1. My ET      — Personalized Newsroom
  2. Navigator  — Interactive Intelligence Briefings
  3. Story Arc  — Narrative Tracking & Knowledge Graph
"""
...
from llm.ollama_client import check_ollama_alive
...
/* Masthead */
.masthead {
    background: linear-gradient(135deg, #0D0D18 0%, #110A05 50%, #0D0A18 100%);
...
def init_session():
    defaults = {
        ...
        "nav_history":       [],   # [{"role": "user"|"assistant", "content": str}]
        "nav_query":         "",
        "arc_result":        None,
        "arc_topic":         "",
...
ollama_status_html = (
    '<span class="status-pill status-live">● LLM ONLINE</span>' if ollama_ok
    else '<span class="status-pill status-warn">⚠ OLLAMA OFFLINE</span>'
)
...
if not ollama_ok:
    st.warning(
        "⚠️ **Ollama is not running.** AI-generated summaries and briefings will be unavailable. "
        "Start Ollama with `ollama serve` and pull the model with `ollama pull llama3.2:3b`."
    )
...
with st.sidebar:
    ...
    persona_icons   = [PERSONAS[p]["icon"] for p in persona_options]
    ...
    with col1:
        if st.button("🌐 Fetch Live", use_container_width=True):
            with st.spinner("Fetching from ET RSS…"):
                progress_bar = st.progress(0)
                def _progress(step, pct):
                    progress_bar.progress(pct)
                result = ingest_articles(use_mock=False, progress_callback=_progress)
            st.success(f"✅ +{result['upserted']} articles")
    ...
    st.caption("**ET AI Intelligence Platform**\nBuilt for ET GenAI Hackathon 2026\n\n"
               "_CPU-only · 16 GB RAM · Offline LLM_")
...
# ════════════════════════════════════════════════════════════════════════════
# TAB 1: MY ET — Personalized Newsroom
# ════════════════════════════════════════════════════════════════════════════
with tab1:
    ...
    if article_count() == 0:
        st.info(
            ...
            "Click **Load Demo** in the sidebar to load 8 high-quality sample articles, "
            "or **Fetch Live** to pull the latest ET news."
        )
    else:
        ...
        if not feed:
            st.warning("No articles matched your current filters. Try broadening the vertical or date range.")
        else:
            # KPI row
            ...
            <div class="kpi-row">
              <div class="kpi-card">
                <div class="kpi-value">{len(feed)}</div>
                <div class="kpi-label">Stories curated</div>
              </div>
              <div class="kpi-card">
                <div class="kpi-value">{market_tone}</div>
                <div class="kpi-label">Market tone</div>
              </div>
              <div class="kpi-card">
                <div class="kpi-value">{len(set(a.get('vertical','') for a in feed))}</div>
            ...
            for article in feed:
                ...
                with st.expander(f"🤖 AI Summary for {persona_key}", expanded=False):
                    if not ollama_ok:
                        st.warning("Ollama offline — start `ollama serve` to enable AI summaries.")
                    else:
...
# ════════════════════════════════════════════════════════════════════════════
# TAB 2: NEWS NAVIGATOR — Interactive Intelligence Briefings
# ════════════════════════════════════════════════════════════════════════════
with tab2:
    ...
    <div class="section-subheader">Multi-document synthesis · Interactive intelligence briefings · Follow-up Q&A</div>
    """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with q_col1:
            # Pre-fill from follow-up if set
            ...
            st.session_state.follow_up_query = ""  # Reset after use
        with q_col2:
            ...
            nav_vertical = st.multiselect(
                "Verticals",
                options=list(ET_RSS_FEEDS.keys()),
                default=[],
                placeholder="All",
                label_visibility="collapsed",
            )
    ...
    if run_briefing and nav_query.strip():
        if article_count() == 0:
            st.error("Knowledge base is empty. Load demo data or fetch live articles first.")
        else:
            st.session_state.nav_history.append({"role": "user", "content": nav_query})
            st.session_state.nav_conversation.add_turn("user", nav_query)

            with st.chat_message("assistant", avatar="📰"):
                with st.spinner("🔍 Retrieving articles and synthesising briefing…"):
                    try:
                        ...
                        result = {
                            "briefing": (
                                f"⚠️ **Briefing generation failed.**\n\n"
                                f"Error: `{_nav_err}`\n\n"
                                "Please ensure Ollama is running (`ollama serve`) "
                                "and the model is loaded (`ollama pull llama3.2:3b`)."
                            ),
                ...
                if not llm_available:
                    st.warning(
                        "⚠️ **Ollama LLM unavailable or slow** — showing article summaries "
                        "instead of a full AI briefing. Start `ollama serve` and ensure "
                        "`llama3.2:3b` is pulled for full synthesis."
                    )
...
with tab3:
        ...
        if article_count() == 0:
            ...
            with st.spinner(f"Building story arc for '{arc_topic}'…"):
                try:
                    ...
                    arc_result = {
                        "error": (
                            f"Story arc analysis failed: `{_arc_err}`\n\n"
                            "Please ensure Ollama is running and `llama3.2:3b` is pulled."
                        )
                    }
    ...
    if arc_result and not arc_result.get("error"):
        ...
        if not llm_ok:
            st.warning(
                "⚠️ **Ollama LLM unavailable or slow** — showing extracted data without "
                "AI narration. Start `ollama serve` for full arc analysis."
            )
        st.success(
        ...
        # Layout: Evolution | Key Players / Sentiment
        evo_col, right_col = st.columns([3, 2])

        with evo_col:
            st.markdown("#### 📖 Evolution Briefing")
            st.markdown(
                f'<div class="briefing-box">{arc_result["evolution_briefing"]}</div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                f'<div class="briefing-box">{arc_result["what_to_watch"]}</div>',
                unsafe_allow_html=True,
            )
        ...
        with right_col:
                    ...
                    st.markdown(
                        ...
                        <div style="margin-bottom:8px;">
                          ...
                          <div style="height:4px;background:var(--border);border-radius:2px;">
                            <div style="height:4px;width:{bar_pct}%;background:var(--accent);border-radius:2px;transition:width 0.4s;"></div>
                          </div>
                        </div>
                        """,
                        unsafe_allow_html=True,
            ...
            if sentiment_traj:
                ...
                fig.add_trace(go.Scatter(
                    x=dates, y=raw, mode="markers",
                    marker=dict(size=8, color=[
                        "#34D399" if s > 0.05 else "#F87171" if s < -0.05 else "#9B96A8"
                        for s in raw
                    ]),
                ...
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(color="#FF6B00", width=2, dash="solid"),
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(gridcolor="#1A1A25", range=[-1, 1]),
                    showlegend=False,
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.caption("Insufficient data for sentiment chart.")
        ...
        if contrarians:
            ...
            st.caption("Articles covering the same events with opposing viewpoints:")
            for pair in contrarians[:3]:
                ...
                with c1:
                    ...
                    st.markdown(
                        f'<div class="news-card" style="border-left-color:{colour};">'
                        f'<div style="font-size:0.7rem;color:{colour};margin-bottom:4px;">{"📈 Bullish" if s > 0.05 else "📉 Bearish"} · {s:.2f}</div>'
                        f'<div class="news-card-title" style="font-size:0.9rem;">{pair["article_a"]["title"]}</div>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                with c2:
                    ...
                    st.markdown(
                        f'<div class="news-card" style="border-left-color:{colour};">'
                        f'<div style="font-size:0.7rem;color:{colour};margin-bottom:4px;">{"📈 Bullish" if s > 0.05 else "📉 Bearish"} · {s:.2f}</div>'
                        f'<div class="news-card-title" style="font-size:0.9rem;">{pair["article_b"]["title"]}</div>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
        ...
        if sentiment_traj:
            for event in sentiment_traj:
                ...
                st.markdown(
                    ...
                    f'<div class="timeline-title">{icon} {event["title"]}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
        else:
        ...
        st.info(
            "🔍 Enter a topic above and click **Track Arc** to build the story's knowledge graph.\n\n"
